...
def profile_view_user(request, username):
    ...
    return render(request, 'core/profile_user.html', {'profile': profile, 'mutual': mutual, 'testimonials': testimonials})

def search(request):
    q = request.GET.get('q','').strip()
    results = User.objects.none()
    if q:
        results = User.objects.filter(username__icontains=q) | User.objects.filter(profile__location__icontains=q) | User.objects.filter(profile__interests__icontains=q)
